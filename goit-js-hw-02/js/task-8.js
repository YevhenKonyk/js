...
//     параметры и проверяет наличие login в массиве allLogins, возвращая true если такого логина еще нет и false 
...
const isLoginValid = function (login) {
    return ogin.length >= 4 && login.length <= 16 ? true : false;
};
...
const addLogin = function (allLogins, login) {
    ...
    if (!isLoginValid(login)) {
        return console.error("Error! Login must contain from 4 to 16 characters");
    }

    if (!isLoginUnique(allLogins, login)) {
        return console.error("The login already used!");
    }

    allLogins.push(login);

    console.info("Login succesfully created");
};

// Test function invokes
addLogin(logins, 'Ajax'); // Success
addLogin(logins, 'robotGoogles'); // Login used
addLogin(logins, 'Zod'); // Range error
addLogin(logins, 'jqueryisextremelyfast'); // Range error